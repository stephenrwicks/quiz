...
const questionForm = (deleteSelf, updateIsEditing) => {
    const form = document.createElement('form');
...
const setup = () => {
    ...
    const updateIsEditing = (isEditing, current) => {
        addQuestionButton.disabled = questionForms.length >= 10;
...
const shortenQuestions = (questions) => {
};
const toBase64 = (questions) => btoa(String.fromCharCode(...new TextEncoder().encode(JSON.stringify(questions))));
const fromBase64 = (str) => JSON.parse(new TextDecoder().decode(Uint8Array.from(atob(str), c => c.charCodeAt(0))));
